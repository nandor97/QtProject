...
TensorProductSurface3::TensorProductSurface3(
    ...
    GLboolean u_closed, GLboolean v_closed):_u_min(u_min), _u_max(u_max),_v_min(v_min), _v_max(v_max),
        _u_closed(u_closed), _v_closed(v_closed), _data(row_count,column_count), _vbo_data(0){}

// homework: copy constructor
TensorProductSurface3::TensorProductSurface3(const TensorProductSurface3& surface):_u_min(surface._u_min), _u_max(surface._u_max),_v_min(surface._v_min), _v_max(surface._v_max),
                                                                              ...
                                                                              GLenum usage_flag) const
{
    ...
    for (GLuint i = 0; i < iso_line_count; i++)
    {
        ...
        GLdouble v;
        // v = min(_v_min + i*v_step, _v_max);
        if (_v_min + i*v_step < _v_max)
            v = _v_min + i*v_step;
        else
            v = _v_max;
        for (GLuint j = 0; j < div_point_count; j++)
        {
            PartialDerivatives pd;
            GLdouble u;
            // u = min(_u_min + j*u_step, _u_max);
            if (_u_min + j*u_step < _u_max)
                u = _u_min + i*u_step;
            else
                v = _v_max;
                                                                              ...
                                                                              GLenum usage_flag) const
{
    ...
    for (GLuint i = 0; i < iso_line_count; i++)
    {
        ...
        for (GLuint j = 0; j < div_point_count; j++)
        {
            PartialDerivatives pd;
            GLdouble v;
            // v = min(_v_min + j*v_step, _v_max);
            if (_v_min + j*v_step < _v_max)
                v = _v_min + j*v_step;
            else
                v = _v_max;
